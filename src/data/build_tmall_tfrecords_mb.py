...
Each TFRecord row = one user's full chronological interaction sequence:
  ...
  - behavior_data : int64 list  (behavior type per item: pv=0, fav=1, cart=2, buy=3)
        ...
        with open(fpath, "r", encoding="utf-8", errors="replace") as f:
            for line in f:
                ...
                try:
                    ...
                    iid_arr[n] = item_id
                    ts_arr[n]  = ts
                    bhv_arr[n] = BEHAVIOR_MAP.get(btype, UNKNOWN_BEHAVIOR)
                    n += 1
    ...
    for i in range(n):
        ...
        if uid != cur_uid:
            if cur_uid is not None:
                commit(cur_uid, cur_seq, cur_bhv)
            cur_uid = uid
            cur_seq = [iid]
            cur_bhv = [bhv]
        else:
